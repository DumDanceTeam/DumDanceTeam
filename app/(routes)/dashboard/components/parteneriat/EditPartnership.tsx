...
interface EditPartnershipProps {
  partnership: Partnership;
}

const EditPartnership: React.FC<EditPartnershipProps> = ({ partnership }) => {
  const [mainImageUrl, setMainImageUrl] = useState(partnership.mainImage);
  const [secondImages, setSecondImages] = useState(partnership.secondImages);
  const [youtubeLink, setYoutubeLink] = useState(partnership.youtubeLink);
  ...
  const { handleSubmit, register, setValue, getValues } = useForm({
    ...
    values: {
      ...
      mainImage: partnership.mainImage,
      secondImages: partnership.secondImages,
      youtubeLink: partnership.youtubeLink,
    },
  });
  ...
  const {mutate: updatePartnership, isLoading} = useMutation({
    mutationFn: async(data)=>{
      const res = await axios.patch(`/api/partnership/${partnership.id}`,{
        title,
        description,
    ...
    onSuccess:()=>{
      toast.success("Parteneriatul a fost actualizat !");
  ...
  return (
    <form onSubmit={handleSubmit(data=>updatePartnership())}>
        <div className="">
          <div className="w-full max-h-[700px] overflow-y-scroll">
            <div>
              <p className="text-center text-[2em] font-medium">Editează parteneriatul</p>
              <p>
                Schimbă datele parteneriatului. Apasă pe butonul salvează când ai
                terminat.
              </p>
            </div>
            <div className="">
