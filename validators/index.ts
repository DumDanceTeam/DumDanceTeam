...
export const ContactValidator = z.object({
    nume: z.string({required_error:"Numele trebuie să conțină cel puțin un caracter !"}).min(1,"Numele trebuie să conțină minim un caracter !"),
    prenume: z.string({required_error:"Prenumele trebuie să conțină cel puțin un caracter !"}).min(1,"Prenumele trebuie să conțină minim un caracter !"),
    email: z.string({required_error:"Email-ul nu poate fi nul !"}).email('Email-ul introdus este invalid'),
    mesaj: z.string({required_error:"Mesajul trebuie să conțină cel puțin un caracter !"}).min(1,"Mesajul trebuie să conțină minim un caracter !"),
});
...
export const AuthTokenValidator = z.object({
    authToken: z.string({required_error:"Introdu un token de acces"}).min(4,"Token-ul trebuie să fie de cel puțin 4 caractere."),
});
...
export const TestmonialUpdateValidator = z.object({
    newData: z.object({
        name: z.optional(z.string().min(1,"Numele trebuie să conțină cel puțin un caracter !")),
        description: z.optional(z.string().min(2,"Descrierea trebuie să conțină cel puțin un caracter !")),
...
export const AppointmentDataValidator = z.object({
    ...
    childAge:z.string({required_error:"Vârsta copilului"}).min(1,"Vârsta copilului este necesară"),
});
