...
interface OnlineGalleryManagerImageProps {
  id: string;
  galleryImageUrl: string;
}
const OnlineGalleryManagerImage: React.FC<OnlineGalleryManagerImageProps> = ({
  id,
  galleryImageUrl,
}: {
  id: string;
  ...
  const { mutate: changeImage, isLoading } = useMutation({
    mutationFn: async () => {
      const res = await axios.patch(`/api/onlineimages/${id}`, { newUrl: imageUrl });
  ...
  const { mutate: deleteImage, isLoading: isLoadingDelete } = useMutation({
    mutationFn: async (id: string) => {
      const res = await axios.delete(`/api/onlineimages/${id}`);
...
export default OnlineGalleryManagerImage;
